...
class SimpleServer(http.server.BaseHTTPRequestHandler):
    #Any function needs to call upon the new and old locations.
    location_list = {}
    """
    ...
    def debug(self, message):
        self.debug_state = False # change this value to False when you submit to the autograder
    ...
    #Documentation: Andrew Wang provided me with a verbal assistance and told me to construct 
    # a function that works on a true false basis. The code within was developed by myself.
    # my code was previously within the 'determine_response' function.
    def check_conditional_get(self):
        if 'If-Modified-Since' in self.headers:
            last_checked_modification = str(self.headers.get("If-Modified-Since"))
            formatted_time = datetime.strptime(last_checked_modification, '%a, %d %b %Y %H:%M:%S %Z')
            last_adjusted_modification = datetime.fromtimestamp(Path(self.path).stat().st_mtime)
            if formatted_time >= last_adjusted_modification:
    ...
    def check_if_moved(self):
        if self.path in self.location_list:
            self.response_code = 301  
            self.updated_location = self.location_list[self.path]
            return True
        return False
    ...
    def determine_response_code(self):
        ...
        if objPath.exists() and objPath.is_file():
            self.path = objPath
            if not self.check_conditional_get(): #Documentation: Andrew Wang helped me develop this statement
                return
            self.response_code = 200
        
        elif not self.check_if_moved(): 
            self.path = basePath / 'notfound.html'
    ...
    def send_response_headers(self):
        ...
        if self.response_code == 200 or self.response_code == 304:
            self.send_header('Last-Modified', self.date_time_string(Path(self.path).stat().st_mtime))
        if self.response_code == 301:
            self.send_header('Location', self.updated_location)
