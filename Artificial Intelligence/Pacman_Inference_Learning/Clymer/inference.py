...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        # put the ghost in jail
        if noisyDistance is None: 
            jailPos = self.getJailPosition()
            self.beliefs = util.Counter()
            self.beliefs[jailPos] = 1.0
            return 
        # Stanard Bayes update for each legal position
        for p in self.legalPositions:
            # True distance if the ghost were at p
            trueDistance = util.manhattanDistance(p, pacmanPosition)
            # Likelihood from the sensor model
            likelihood_its_there = emissionModel[trueDistance]
            # Posterier is prior times likelihood
            allPossible[p] = self.beliefs[p] * likelihood_its_there
        "*** END YOUR CODE HERE ***"
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        # Gotta hanlde all positions with a nonzero prior probability 
        for oldPos, oldProb in self.beliefs.items():
            if oldProb == 0:
                continue
            # Ensure the jailed ghost stays in jail
            if oldPos == self.getJailPosition():
                nextBeliefs[oldPos] += oldProb 
                continue 
            # Build a temp game state with the ghost at the old position
            tempState = self.setGhostPosition(gameState,oldPos)
            # Get model for a distribution of potential next positions
            newPosDist = self.getPositionDistribution(tempState)
            # Distribute oldProbability over the possible next positions
            for newPosition, transitionProbability in newPosDist.items():
                nextBeliefs[newPosition] += oldProb * transitionProbability
        # Normalize for a valid distribution
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        numLegal = len(legal)
        # Cycle through legal positions until we have the request number of particles
        for i in range(numParticles):
            position = legal[i % numLegal] # loops back to start of list
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is None:
            ...
            return 
        # Weight each particle by likelihood
        belief = util.Counter()
        for particlePos in self.particles: 
            ...
            belief[particlePos] += likelihood
        # Check if all weights are zero, if so then restart uniformly
        if belief.totalCount() == 0: 
            self.initializeUniformly(gameState)
            return 
        
        # Normalize and draw a new particle set by sampling with replacement
        belief.normalize()
        newParticles = []
        for i in range(self.numParticles): 
            sampledPos = util.sample(belief)
    ...
    def elapseTime(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        # Build list of future positions
        newParticles = []
        # loop through all existing particle
        for oldPosition in self.particles: 
            # keep jail in place
            if oldPosition == self.getJailPosition():
                newParticles.append(oldPosition)
                continue 
            # Get distro over new positions for the ghost
            newPositionDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPosition))
            # Randomly sample a new position based on probability
            newParticle = util.sample(newPositionDist)
            # Store sample
    ...
    def getBeliefDistribution(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        beliefDistro = util.Counter()
        # count how many times each position appears adn tally them
        for particle in self.particles: 
            beliefDistro[particle] += 1.0
        # convert to probability
        beliefDistro.normalize()
        # return
        return beliefDistro
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        # All joint tuples of length numGhosts, ensured that ghosts can share the same space.
        allTuples = list(itertools.product(self.legalPositions, repeat = self.numGhosts))
        # Shuffle so that tiling below spread out evenly
        random.shuffle(allTuples)
        n = len(allTuples)
        # tile all the shuffled list to reach the numparticles total particles
        particles = []
        for i in range(self.numParticles):
            index = i % n # wrap if overlap
            particle = allTuples[index]
    ...
    def observeState(self, gameState):
        ...
        for particle in self.particles: 
            updated = particle
            # enforce jail for any captured ghosts
            for i in range(self.numGhosts):
                if noisyDistances[i] is None:
                    # if ghost i is captured, force its position to jail particle
                    updated = self.getParticleWithGhostInJail(updated, i)
            # Compute the weight for each updated particle
            weight = 1.0
            for i in range(self.numGhosts):
                # Handle jailed ghosts
                if noisyDistances[i] is None: 
                    continue 
                # get true distance from Pacman to the particle's position for ghost i
                trueDistance = util.manhattanDistance(updated[i], pacmanPosition)
                # multiply by likelihood of observing the noisy distance given true distance
                weight *= emissionModels[i][trueDistance]
            # Accumulate weight for this particle key
            beliefs[updated] += weight
        # if all beliefs are zero then
        if beliefs.totalCount() == 0:
            self.initializeParticles()
            # After the reinitialization, enforce jail for any currently captured ghosts
            newParticles = []
            for particle in self.particles:
                for i in range(self.numGhosts):
                    if noisyDistances[i] is None: 
        ...
        #Normalize wieghts and resample a fresh particle list with replacement
        beliefs.normalize()
        particles = []
        for i in range(self.numParticles):
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            "*** YOUR CODE HERE ***"
            for i in range(self.numGhosts):
                # grab a list of the next pos for ghost i bc it depends on all ghosts prev pos
                prevGhostPosition = list(newParticle)
                # Build game state with ghosts set to the previous position then get the i-th ghost for position distro
                newPostionDistance = getPositionDistributionForGhost(setGhostPositions(gameState, prevGhostPosition), i, self.ghostAgents[i])
                # Sample a new position for ghost i from that distribution
                newParticle[i] = util.sample(newPostionDistance)
